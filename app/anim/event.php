<?php   
    ...
    $sql = 'SELECT * FROM absent WHERE event_id = :event_id';
     $preparedStatement = $connexion->prepare($sql);
     $preparedStatement->bindValue(':event_id',  $event_id['id']);
     $preparedStatement->execute();
     $absent_info = $preparedStatement->fetchAll(PDO::FETCH_ASSOC);  
...
<section class='event-info  info visible info-absent ' >
  ...
  <?php 
    ...
    if (empty($item_list)) {
        ...
        echo "<h3 class='event-info-title'> Ils viennent tous ! </h3>";
    } else {
    $i = 0;
    echo "<h3 class='event-info-title'> Ils ne participeront pas </h3>

  <ul  class='absent-list'> ";
    foreach($absent_info as $row) {

        echo "

        <li><p>". $row['absent_info'] ."</p></li> 
...
<fieldset class="item-list"> 
  ...
  <?php 
    ...
    if (empty($item_list)) {
        ...
        echo "<p> Ne prépare rien, tes chefs s'en occupe. </p>";
    } else {
    $i = 0;
...
<a href='create-event.php' class='edit-btn call-to-action-btn'></a>
